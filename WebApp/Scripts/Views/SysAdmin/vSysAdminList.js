...
async function changeStatusAdmin(data, data2, button) {
	...
	ctrlActions.PutToAPI(sysAdminList.service + "/changeStatus", adminData, function () {
		...
		let btnActions = '<button class="btn btn-primary"  style="margin-right: 10px;" title="Actualizar Administrador" onclick="updateAdmin(' + adminData.SysAdminUserID + ')" id="updateAdmin"><i class="fa fa-user-edit"></i></button ><button ' + (data[4] == 'Activo' ? 'class="btn btn-danger"' : 'class="btn btn-success"') + ' id="changeStatus' + adminData.SysAdminUserID + '" title="' + (data[i].UserActiveStatus == 'Activo' ? 'Desactivar Admin' : 'Activar Admin') + '" onclick="changeStatusAdmin(' + adminData.SysAdminUserID + ',\'' + data[4] + '\', this)">' + (data[4] == 'Activo' ? '<i class="fa fa-eye-slash"></i>' : '<i class="fa fa-eye"></i>') + '</button >';
